-- Full Supabase Database Schema for Banking App
...
CREATE TABLE IF NOT EXISTS card_requests (
  ...
  card_tier VARCHAR(20) NOT NULL CHECK (card_tier IN ('standard', 'gold', 'platinum')),
...
DROP POLICY IF EXISTS "Users can update their own deposit requests" ON deposit_requests;
...
CREATE POLICY "Users can insert their own card requests"
  ON card_requests FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own card requests" ON card_requests;
CREATE POLICY "Users can update their own card requests"
...
CREATE POLICY "Admins can create crypto wallets"
...
-- Trigger for crypto_wallets updated_at
